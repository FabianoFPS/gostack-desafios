...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function checksFavorite(): Promise<void> {
      try {
        ...
        const foddFilter = response.data.filter(
          foodRes => foodRes.id === food.id,
        );

        if (foddFilter.length) {
          setIsFavorite(true);
        }
      } catch (error) {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      try {
        ...
        const response = await api.get<OrderFood>(`foods/${id}`);
        const { data: orderFood } = response;
        setFood({
          ...orderFood,
          formattedPrice: formatValue(orderFood.price),
        });
        setExtras(
          orderFood.extras.map((extra: Omit<Extra, 'quantity'>) => ({
            ...extra,
            quantity: 0,
          })),
        );
  ...
  const cartTotal = useMemo(() => {
    const extraTotal: number = extras.reduce((accumulator, extra) => {
